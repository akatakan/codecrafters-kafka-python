...
def main():
    ...
    connection.sendall(message_size+corr_id.to_bytes(24, signed=True)+(0).to_bytes(2, signed=True)+api_keys+api_key.to_bytes(2, signed=True)+(0).to_bytes(2, signed=True)+ (4).to_bytes(2, signed=True))
